...
/** ClickedX is one of the callbacks for GUI controls.
   ...
   static void ClickedX(Fl_Widget * w, void * userdata) {
     ...
     std::ofstream _ofs("media.json",std::ofstream::trunc); //need to fix
     _ofs.close();//need to fix - attempt is to delete what's on media.json
   ...
   static void SearchCallbackS(Fl_Widget*w, void*data) {
      ...
      try{
         std::string url = "http://ws.audioscrobbler.com/2.0/?method=album.getinfo&artist=BTS&album=map+of+the+soul+:+Persona&format=json&api_key=";
         url = url + o->lastfmkey;
	 //------------------editing 
	 ...
	 cout << "searching album... "<< _albumname <<endl;
	 string base  = _albumname;
	 string addstr = "+";
	 //cout << "counting length... "<< base.length() << endl;
	 //cout <<"first letter : " << base[0] << endl;
	 for ( int i = 0; i <base.length()-1; i++ ){
	   if ( base[i]==' '){
	     //cout<<"found space at "<< i << endl;
	     base.replace(i,1,addstr);
	   }

	 }
	 cout<< "new name: " << base << endl;
	 url = _pre + _artistname +_mid + base + _post + o->lastfmkey;
	 //------------------------editing 
	 ...
	 outfile << std::setw(4) << str << std::endl;
	 Track* x;
	 x->jsonreadwrite();
   ...
   void TreeCallback() {
     ...
     // Track* x;
      ...
      case   FL_TREE_REASON_SELECTED: {
         ...
         cout << "media: "<< md.title << " " << md.album << " " <<md.time<<" "
              << md.author << " " << md.rank << " " << md.typeExt
              << endl;
         trackInput->value(md.title.c_str());
         albumInput->value(md.album.c_str());
         timeInput->value(md.time.c_str());
