...
/** ClickedX is one of the callbacks for GUI controls.
   ...
   void Menu_Clicked() {
	...
	library->toJsonFile("mediacollection.json");
	...
	std::stringstream stream;
	 ...
	 cout << ".... REMOVE TRACK FUNCTION END " << endl;
      }else if(selectPath.compare("Track/Remove")==0){//...........IMPLEMNTING
	cout << "...........STARTING REMOVE TRACK. "  << endl;
	
	cout << "inserting "<< item->label()<<" to be removed"<< endl;
	library->removeTrack(item->label());
	
	item->parent()->remove_child(item->label());
	tree->redraw();
	cout << "REMOVE TRACK ENDED. " << endl;
